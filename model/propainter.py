...
class BidirectionalPropagation(nn.Module):
    ...
    def forward(self, x, flows_forward, flows_backward, mask, interpolation='bilinear'):
        ...
        if self.learnable:
            ...
            # outputs = self.fuse(torch.cat([outputs_b, outputs_f, mask_in], dim=1)) + x.view(-1, c, h, w)  # default
            outputs = self.fuse(torch.cat([outputs_b, outputs_f, mask_in], dim=1)) + x.reshape(-1, c, h, w).contiguous()
        else:
